...
// Indexa TEXTOS avulsos sob uma chave qualquer (ex.: conhecimento de um agente).
// items: [{ title, content }]. Usa o mesmo formato de índice da pasta, então
// search()/has()/remove() funcionam igual.
async function indexTexts(key, items = [], { maxChunks = 4000 } = {}) {
  ensure();
  const pieces = []; // {file, idx, text}
  for (const it of items) {
    const title = String(it.title || "conhecimento");
    const chunks = chunkText(String(it.content || ""));
    chunks.forEach((c, i) => { if (pieces.length < maxChunks) pieces.push({ file: title, idx: i, text: c }); });
  }
  if (!pieces.length) throw new Error("Nenhum conteúdo de texto para indexar.");

  const vecs = [];
  const BATCH = 64;
  for (let i = 0; i < pieces.length; i += BATCH) {
    const part = await providers.embedTexts(pieces.slice(i, i + BATCH).map((x) => x.text), { taskType: "RETRIEVAL_DOCUMENT" });
    for (const v of part) vecs.push(normalize(v));
  }
  ...
  const index = {
    ...
    dim: vecs[0]?.length || 0,
    builtAt: new Date().toISOString(),
    files: new Set(pieces.map((x) => x.file)).size,
    chunks: pieces.map((it, k) => ({ file: it.file, idx: it.idx, text: it.text, vec: vecs[k] })),
  };
  fs.writeFileSync(pathFor(key), JSON.stringify(index), "utf8");
  return { chunks: index.chunks.length, files: index.files, builtAt: index.builtAt };
}
...
module.exports = { ensure, status, has, indexProject, indexTexts, search, remove, chunkText };
